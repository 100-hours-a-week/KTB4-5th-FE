...
| `ExpirationDateSheet`           | 공통 Feature                      | `src/features/select-expiration-date`; `REG-005`와 `STOCK-002`의 공통 유통기한 선택 UI와 날짜 범위를 소유한다. 직접 열지 않고 `IngredientExpirationDateInput`이 조합한다.                                                    |
| `IngredientExpirationDateInput` | 공통 Feature                      | `src/features/select-expiration-date`; 유통기한 선택 버튼 표현, 날짜 표시 형식, `ExpirationDateSheet`의 open 상태와 선택 후 닫기를 소유하고, React Hook Form의 필드 경로·오류 판정과 label·helper 조합은 각 Page가 소유한다. |
