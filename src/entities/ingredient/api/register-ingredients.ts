...
/**
 * 일괄 등록 요청 한 건. 수량과 무게는 measureType에 따라 한쪽만 값을 가진다.
 * imageUploadId는 영수증·실물 사진으로 등록할 때만 채운다.
 */
...
/**
 * 재고를 일괄 등록한다. 한 요청 전체가 하나의 트랜잭션이라 한 건이라도 검증에
 * 걸리면 아무것도 저장되지 않으며, 항목별 오류는 응답으로 내려오지 않는다.
 * 이름·보관 방법·유통기한·측정 방식·단위가 모두 같은 기존 재고에는 새 행을
 * 만들지 않고 수량을 합산한다.
 */
